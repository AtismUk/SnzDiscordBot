...
public interface IEventService
{
    ...
    Task<EventEntity?> UpdateEventAsync(ulong guildId, ulong channelId, ulong messageId, 
        DateTime? startAt = null);

    Task<EventEntity?> RemoveEventAsync(ulong guildId, ulong channelId, ulong messageId);

    Task<(EventEntity?, MemberEntity?, EventVoteEntity?)> UpdateVoteAsync(ulong guildId, ulong channelId, ulong messageId, ulong userId, 
        VoteType voteType);
